...
class Neo4jConnection:
    ...
    def __init__(self):
        ...
        # Clean URI if it contains credentials
        if "@" in self.uri:
            protocol_part = self.uri.split("://")[0]
            host_part = self.uri.split("@")[-1]
            self.uri = f"{protocol_part}://{host_part}"
...
def init_neo4j_connection():
    """Initialize and cache Neo4j connection"""
    conn = Neo4jConnection()
    conn.connect()
    return conn

def get_data_overview(neo4j_conn):
...
def create_network_visualization(nodes, edges, height=600):
    ...
    net = Network(height=f"{height}px", width="100%", bgcolor="#ffffff", font_color="#000000")
...
def main():
    ...
    # Header
    st.markdown('<h1 class="main-header">📚 Research Paper Network Explorer</h1>', 
                unsafe_allow_html=True)
    ...
    if not neo4j_conn.connected:
        st.error("❌ Could not connect to Neo4j database. Please check your connection settings.")
    ...
    with st.sidebar:
        ...
        with st.spinner("Loading data overview..."):
            overview = get_data_overview(neo4j_conn)
    ...
    with tab1:
        ...
        if overview.get('papers', 0) == 0:
            st.warning("⚠️ No data found in the database. Please run your scraper first to populate the database.")
            st.info("Make sure your scraper has successfully imported data into Neo4j before using this application.")
            return
        ...
        with col4:
            avg_papers_per_volume = (overview.get('papers', 0) / max(overview.get('volumes', 1), 1))
            st.metric(
                "Avg Papers/Volume",
        ...
        year_data = neo4j_conn.execute_query(year_query)
    ...
    with tab2:
        ...
        with col1:
            search_term = st.text_input("🔍 Search papers by title", 
        ...
        papers_data = neo4j_conn.execute_query(papers_query, parameters)
    ...
    with tab3:
        ...
        authors_query = """
        MATCH (person:Person)-[r:AUTHORED]->(paper:Paper)
        RETURN person.name as name, count(paper) as paper_count
        ORDER BY paper_count DESC
        LIMIT 20
        """
        
        authors_data = neo4j_conn.execute_query(authors_query)
        
        if authors_data:
            df_authors = pd.DataFrame(authors_data)
            fig_authors = px.bar(df_authors.head(10), x='paper_count', y='name', 
                               orientation='h',
        ...
        editors_data = neo4j_conn.execute_query(editors_query)
    ...
    with tab4:
        ...
        viz_type = st.selectbox(
            ...
            ["Author Collaboration Network", "Paper-Volume Network", "Keyword Co-occurrence"]
        )
        ...
        if viz_type == "Author Collaboration Network":
            ...
            collab_data = neo4j_conn.execute_query(collab_query, {'min_collab': min_collaborations})
            ...
            if collab_data:
                ...
                if nodes and edges:
                    st.success(f"Showing {len(nodes)} authors with {len(edges)} collaborations")
                    html_content = create_network_visualization(nodes, edges)
                    st.components.v1.html(html_content, height=600)
                else:
                    st.info(f"No collaborations found with minimum {min_collaborations} shared papers.")
            else:
            ...
            pv_data = neo4j_conn.execute_query(pv_query, {'limit': volume_limit})
            ...
            if pv_data:
                ...
                for volume in volumes:
                    nodes.append({
                        'id': volume,
                        'label': volume[:50] + "..." if len(volume) > 50 else volume,
                ...
                if nodes and edges:
                    st.success(f"Showing {len(volumes)} volumes with {len(papers)} papers")
                    html_content = create_network_visualization(nodes, edges)
                    st.components.v1.html(html_content, height=600)
            else:
    ...
    with tab5:
        ...
        selected_query = st.selectbox("Choose a sample query or write your own:", 
        ...
        query = st.text_area("Cypher Query:", value=query_text, height=100,
        ...
        with col2:
            if st.button("ℹ️ Show Schema"):
                ...
                """
                try:
                    schema_result = neo4j_conn.execute_query(schema_query)
                    if schema_result:
                        st.json(schema_result)
                    else:
                        # Fallback schema query
                        labels_result = neo4j_conn.execute_query("CALL db.labels()")
                        rels_result = neo4j_conn.execute_query("CALL db.relationshipTypes()")
                        ...
                        if rels_result:
                            st.write([record['relationshipType'] for record in rels_result])
                except:
